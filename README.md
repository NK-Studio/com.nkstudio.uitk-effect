...
Package Manager에서 UITK Effects를 선택한 뒤 **Samples > Filter Sample > Import**를 클릭합니다. 샘플에는 그림자, 글로우, 아웃라인, 그라디언트가 조합된 UI와 실행 가능한 씬이 포함되어 있습니다.

## Unity 6.5

샘플 씬은 Unity 6.3 이상에서 열 수 있도록 `UIDocument` 기반으로 저장됩니다. Unity 6.5 이상에서는 샘플에 포함된 에디터 도우미가 씬을 열 때 `PanelRenderer`를 사용할 수 있도록 자동으로 업그레이드합니다. 필터와 머티리얼 사용 방식은 두 구성에서 동일합니다.
